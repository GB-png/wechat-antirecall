...
final class InlineHookEngineTests: XCTestCase {
    ...
    func testEncoderMatchesRecorded269619StaticPatches() throws {
        var revokeBytes = [UInt8](repeating: 0, count: 12)
        XCTAssertEqual(wechat_antirecall_encode_entry_stub(0x497f908, 0x9f9ff00, &revokeBytes), 1)
        XCTAssertEqual(revokeBytes.map { String(format: "%02X", $0) }.joined(), "10B10290108247F900021FD6")
        XCTAssertEqual(wechat_antirecall_decode_entry_stub_slot(&revokeBytes, 0x497f908), 0x9f9ff00)

        var messageBytes = [UInt8](repeating: 0, count: 12)
        XCTAssertEqual(wechat_antirecall_encode_entry_stub(0x491cb1c, 0x9f9ff08, &messageBytes), 1)
        XCTAssertEqual(messageBytes.map { String(format: "%02X", $0) }.joined(), "10B402F0108647F900021FD6")
    ...
    func testEncoderMatchesRecorded269628StaticPatches() throws {
        var revokeBytes = [UInt8](repeating: 0, count: 12)
        XCTAssertEqual(wechat_antirecall_encode_entry_stub(0x49aef90, 0xa043f00, &revokeBytes), 1)
        XCTAssertEqual(revokeBytes.map { String(format: "%02X", $0) }.joined(), "B0B402B0108247F900021FD6")
        XCTAssertEqual(wechat_antirecall_decode_entry_stub_slot(&revokeBytes, 0x49aef90), 0xa043f00)

        var messageBytes = [UInt8](repeating: 0, count: 12)
        XCTAssertEqual(wechat_antirecall_encode_entry_stub(0x494c1a4, 0xa043f08, &messageBytes), 1)
        XCTAssertEqual(messageBytes.map { String(format: "%02X", $0) }.joined(), "B0B702F0108647F900021FD6")
        XCTAssertEqual(wechat_antirecall_decode_entry_stub_slot(&messageBytes, 0x494c1a4), 0xa043f08)
    }
    ...
    func testMessageCaptureTrampolinePreservesConditionalFlags() throws {
        XCTAssertEqual(
            ...
            "269340/269341/269574/269575/269576/269577/269578/269579/269619/269624/269628 Message-finalizer trampoline did not preserve the ccmp flags"
        )
    }
}
