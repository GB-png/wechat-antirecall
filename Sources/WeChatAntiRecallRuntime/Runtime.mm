...
constexpr InlineRevokeHookConfig inlineRevokeHookConfigs[] = {
    ...
    // 269574 (WeChat 4.1.13): normalized full-function matching against the official
    // 269341 arm64 slice has a single strong match at 0x4905488. The +0x270 guard and
    // +0xA10 newmsgid store keep the same geometry, while the output layout moves to
    // newMsgId=0x1C8 and replaceMsg=0x1D0. The entry stub targets SLOT 0x9ecbf00.
    {"269574", 0x4905488, {0xA9BC5FF8, 0xA90157F6, 0xA9024FF4}, 0x4905494, 0x1c8, 0x1d0},
    // 269575 (WeChat 4.1.13 hotfix): the 269574 geometry remains the unique full-slice
    // match and relocates uniformly by -0x5B8. The +0x270 guard and +0xA10 newmsgid
    ...
    // 269578 (WeChat 4.1.13.10): the 269574+ geometry remains the unique full-slice
    // match and relocates uniformly by +0x28 from 269577. The +0x270 guard and
    // +0xA10 newmsgid store stay put, and this build independently decodes the
    // same output fields (newMsgId=0x1C8, replaceMsg=0x1D0). SLOT remains
    // 0x9f57f00; the entry stub is unchanged because the parser stayed on the
    // same 4K page.
    {"269578", 0x495884c, {0xA9BC5FF8, 0xA90157F6, 0xA9024FF4}, 0x4958858, 0x1c8, 0x1d0},
    // 269579 (WeChat 4.1.13.11): the 269574+ geometry remains the unique full-slice
    // match and relocates uniformly by -0x1C from 269578. The +0x270 guard and
    // +0xA10 newmsgid store stay put, and this build independently decodes the
    // same output fields (newMsgId=0x1C8, replaceMsg=0x1D0). SLOT remains
    // 0x9f57f00; the entry stub is unchanged because the parser stayed on the
    // same 4K page.
    {"269579", 0x4958830, {0xA9BC5FF8, 0xA90157F6, 0xA9024FF4}, 0x495883c, 0x1c8, 0x1d0},
    // 269619 (WeChat 4.1.13.51): the 269574+ geometry remains the unique full-slice
    // match. The parser relocates by +0x270D8 from 269579 to 0x497f908; the
    // +0x270 guard and +0xA10 newmsgid store stay put, and this build
    // independently decodes the same output fields (newMsgId=0x1C8,
    // replaceMsg=0x1D0). SLOT moves with __DATA to 0x9f9ff00.
    {"269619", 0x497f908, {0xA9BC5FF8, 0xA90157F6, 0xA9024FF4}, 0x497f914, 0x1c8, 0x1d0},
    // 269624 (WeChat 4.1.13.56): the 269574+ geometry remains the unique full-slice
    // match. The parser and finalizer both relocate uniformly by +0x2E3C0 from
    // 269619; the +0x270 guard and +0xA10 newmsgid store stay put, and this
    // build independently decodes the same output fields (newMsgId=0x1C8,
    // replaceMsg=0x1D0). SLOT moves with __DATA to 0xa03ff00.
    {"269624", 0x49adcc8, {0xA9BC5FF8, 0xA90157F6, 0xA9024FF4}, 0x49adcd4, 0x1c8, 0x1d0},
    // 269628 (WeChat 4.1.13.60): the 269574+ geometry remains the unique full-slice
    // match. The parser and finalizer both relocate uniformly by +0x12C8 from
    // 269624; the +0x270 guard and +0xA10 newmsgid store stay put, and this
    // build independently decodes the same output fields (newMsgId=0x1C8,
...
constexpr InlineMessageCaptureHookConfig inlineMessageCaptureHookConfigs[] = {
    ...
    // 269574: the network constructor at 0x48A27B8 still copies serverId to +0xF8,
    // content to +0x130, and routes msgType through 0x48A3494 to +0x0C before its
    // unconditional call to the common finalizer at 0x48A3B20. The finalizer keeps
    // the ldrb/cmp/ccmp entry shape; only the finalized flag moves from +0x250 to +0x258.
    {
        "269574",
        0x48a3b20,
    ...
    // 269575: the network constructor at 0x48A2200 still copies serverId to +0xF8,
    // content to +0x130, and routes msgType through 0x48A2EDC to +0x0C before its
    // unconditional call to the common finalizer at 0x48A3568. The finalizer keeps
    // the ldrb/cmp/ccmp entry shape and finalized flag at +0x258.
    {
        "269575",
        0x48a3568,
        {0x39496008, 0x7100051F, 0x7A400820},
    ...
    // 269578: the network constructor at 0x48F519C still copies serverId to +0xF8
    // (str x8,[x19,#0xF8]), routes msgType through 0x48F5E78 to +0x0C, and
    // unconditionally calls the common finalizer at 0x48F6504. The finalizer
    // keeps the ldrb/cmp/ccmp entry shape, finalized flag at +0x258, and SLOT
    // 0x9f57f08.
    {
        "269578",
    ...
    // 269579: the network constructor at 0x48F5180 still copies serverId to +0xF8
    // (str x8,[x19,#0xF8]), routes msgType through 0x48F5E5C to +0x0C, and
    // unconditionally calls the common finalizer at 0x48F64E8. The finalizer
    // keeps the ldrb/cmp/ccmp entry shape, finalized flag at +0x258, and SLOT
    // 0x9f57f08.
    {
        "269579",
    ...
    // 269619: the network constructor at 0x491B7C4 still copies serverId to +0xF8
    // (ldr x8,[x23,#0x50] / str x8,[x19,#0xF8]), content to +0x130
    // (add x0,x19,#0x130), and routes msgType through 0x491C490 to +0x0C
    // before its unconditional call to the common finalizer at 0x491CB1C.
    // The finalizer keeps the ldrb/cmp/ccmp entry shape, finalized flag at
    // +0x258, and SLOT 0x9f9ff08.
    {
        "269619",
        0x491cb1c,
        {0x39496008, 0x7100051F, 0x7A400820},
    ...
    // 269624: the network constructor at 0x4949B84 still copies serverId to +0xF8
    // (ldr x8,[x23,#0x50] / str x8,[x19,#0xF8]), content to +0x130
    // (add x0,x19,#0x130), and routes msgType through 0x494A850 to +0x0C
    // before its unconditional call to the common finalizer at 0x494AEDC.
    // The finalizer keeps the ldrb/cmp/ccmp entry shape, finalized flag at
    // +0x258, and SLOT 0xa03ff08.
    {
        "269624",
        0x494aedc,
        {0x39496008, 0x7100051F, 0x7A400820},
    ...
    // 269628: the network constructor at 0x494AE4C still copies serverId to +0xF8
    // (ldr x8,[x23,#0x50] / str x8,[x19,#0xF8]), content to +0x130
    // (add x0,x19,#0x130), and routes msgType through 0x494BB18 to +0x0C
    // before its unconditional call to the common finalizer at 0x494C1A4.
    // The finalizer keeps the ldrb/cmp/ccmp entry shape, finalized flag at
    // +0x258, and SLOT 0xa043f08.
    {
        "269628",
        0x494c1a4,
        {0x39496008, 0x7100051F, 0x7A400820},
