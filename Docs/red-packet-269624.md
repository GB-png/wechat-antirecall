# 红包调用链：macOS 微信 269624 / 269628

2026-09-06，IDA Professional 9.4 / Hex-Rays arm64，静态分析本机微信 4.1.13.56（269624）的安装前备份。2026-09-09 将同一组入口按几何指纹与 ADRP 解码重定位到 4.1.13.60（269628）。

实现状态：已接入消息 hook、原生接收/拆包任务、串行队列和 GUI/CLI 开关。编译及离线检查通过，**尚未进行真实红包领取验证**。默认关闭，只适配 269624 与 269628。这里记录的是红包业务调用链，未重写微信的登录、加密传输或支付认证协议。运行时按构建选择地址表；九处原生指纹不一致时不会调用这些地址。
...
- 仅在 269624 / 269628 且该构建九处原生函数指纹一致时初始化；其他构建不调用这些地址。
...
静态分析和离线 ABI 测试不能证明真实微信运行中的完整兼容性，也不能证明实际领取成功。
